...
sub InsertUser() {
	...
	# Getting new user_id is DB-dependant, so do the right thing for the DB
	if($db_driver eq "Oracle") {
		# Can't easily get the new ROWID from Oracle, so do it manually with
		# the sequence instead.
		$sth = &ExecSQL(
			"SELECT seq_users_user_id.NEXTVAL AS new_user_id " .
			"FROM dual"
			);
		...
		$sth = &ExecSQL(
			"INSERT INTO users " .
				"(user_id, user_name, user_name_lower, password, email, email_private, " .
				"url, status, create_utime) " .
			"VALUES " .
				"($new_user_id, '$user_name', '$user_name_lower', '$password', '$email', " .
...
sub SavePassword() {
	...
	$old_password   = &DBEscapeString(trim(param('old_password')));
	$new_password_1 = &DBEscapeString(trim(param('new_password_1')));
	$new_password_2 = &DBEscapeString(trim(param('new_password_2')));
...
sub SendPassword() {
	...
	# Verify format
	unless(&EmailOK($email)) {
		&SendPasswordScreen($ls_bad_email_format{$language});
		&CleanExit(0);
	}
...
sub AddRecipeBookmark() {
	...
	# Make sure recipe id is numeric
	unless($recipe_id =~ /^\d+$/) {
