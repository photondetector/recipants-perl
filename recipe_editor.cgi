...
if($cmd ne "new" && $cmd ne "save_new" && ! $recipe_id) {
	&PrintErrorExit($ls_no_recipe_id{$language});
}

# We've got a legit command and a recipe_id (if necessary), set up the template
...
#	$dbh->rollback();
	...
	if($cmd eq "save_edited") {
		$page_title = $ls_edit_recipe{$language} . param('name');
	} else {
...
sub InsertRecipeBaseInfo() {
	...
	if($new_recipe_id) {
		...
		&PrintErrorExit("InsertRecipeBaseInfo(): " .
			$ls_cant_get_new_recipe_id{$language});
	}
}
